...
export const decodeMimeEncodedWords = (value: string) =>
...
export const extractEmailAddresses = (text: string) => {
...
export const parseReceivedAt = (rawDate?: string) => {
...
export const normalizeBodyText = (raw: string, transferEncoding?: string) => {
...
export const stripMailHeadersFromPreview = (text: string) => {
...
export const buildInboxPreview = (raw: string) => {
...
const attachSocketTimeout = (socket: tls.TLSSocket) => {
  socket.setTimeout(SOCKET_TIMEOUT_MS);
  socket.once('timeout', () => {
    try { socket.destroy(new Error('IMAP socket timed out')); } catch { /* noop */ }
  });
};
