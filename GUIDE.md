...
**Domain request modal:** Users can submit add/remove requests from the public "Request domain" modal with Turnstile protection. The modal stores the request UUID locally and includes a "Check Status" tab. When a request is approved, the status check returns the actual Cloudflare-assigned nameservers from the onboarding record. Admins can enable "Auto-approve domain add requests" in the admin panel to create Cloudflare zones immediately and show nameservers to users without manual approval. Removal requests always stay admin-reviewed.
